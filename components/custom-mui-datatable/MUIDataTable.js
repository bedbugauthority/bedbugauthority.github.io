...
class MUIDataTable extends React.Component {
    ...
    columns.forEach((column, colIndex) => {
      ...
      for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
        ...
        if (typeof tableData[rowIndex] === "undefined") {
          tableData.push({
            index:
              status === TABLE_LOAD.INITIAL ? rowIndex : data[rowIndex].index,
            data:
              status === TABLE_LOAD.INITIAL
                ? data[rowIndex]
                : data[rowIndex].data,
        ...
        if (typeof columnOptions.customBodyRender === "function") {
          const tableMeta = this.getCustomRenderMeta(
            rowIndex,
            colIndex,
            value,
            [],
  ...
  computeDisplayRow(columns, row, rowIndex, filterList, searchText) {
    ...
    for (let index = 0; index < row.length; index++) {
      ...
      if (
        ...
        filterList[index].indexOf(columnValue) < 0
      ) {
        isFiltered = true
      }

      const searchCase = !this.options.caseSensitive
      ...
      (prevState) => {
        ...
        for (let pos = 0; pos < columns.length; pos++) {
          if (index !== pos) {
            ...
            columns[pos].sortDirection =
              columns[pos].sortDirection === "asc" ? "desc" : "asc"
          }
        }

        const orderLabel = this.getSortDirection(columns[index])
        const announceText = `Table now sorted by ${
          columns[index].name
        } : ${orderLabel}`
  ...
  handleFilterUpdate = (index, column, type) => {
    this.setState((prevState) => {
  ...
  handleSelectRowUpdate = (type, value) => {
    if (type === "head") {
      this.setState(
        (prevState) => {
        ...
        () => {
          if (this.options.onRowsSelect) {
            this.options.onRowsSelect(
              this.state.curSelectedRows.data,
  ...
  handleCellClick = (colIndex, rowIndex) => {
    const message = "clicked cell: " + colIndex + "," + rowIndex
    alert(message)
  }

  sortCompare(order) {
    return (a, b) =>
      (typeof a.data.localeCompare === "function"
        ? a.data.localeCompare(b.data)
        : a.data - b.data) * (order === "asc" ? -1 : 1)
  }
  ...
  render() {
    console.log("State on MUIDataTable render: ", this.state)
      ...
      >
        {selectedRows.data.length ? (
          <MUIDataTableToolbarSelect
            ...
            onRowsDelete={this.handleSelectRowDelete}
          />
        ) : (
          <MUIDataTableToolbar
            title={title}
            ...
            onSearchTextUpdate={this.handleSearchTextUpdate}
        ...
        >
          ...
          >
            ...
            <MUIDataTableHead
              ...
              selectedRows={selectedRows}
              onSelectRowUpdate={this.handleSelectRowUpdate}
...
export default withStyles(tableStyles)(MUIDataTable)
